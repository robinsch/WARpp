...
uint32 DatabaseWorkerPool<T>::Open()
{
    ...
    WAR_LOG_INFO("sql.driver", "Opening DatabasePool '%s'. "
    ...
    if (!error)
    {
        WAR_LOG_INFO("sql.driver", "DatabasePool '%s' opened successfully. " SZFMTD
...
{
    WAR_LOG_INFO("sql.driver", "Closing down DatabasePool '%s'.", GetDatabaseName());

    //! Closes the actualy MySQL connection.
    _connections[IDX_ASYNC].clear();

    WAR_LOG_INFO("sql.driver", "Asynchronous connections on DatabasePool '%s' terminated. "
    ...
    WAR_LOG_INFO("sql.driver", "All connections on DatabasePool '%s' closed.", GetDatabaseName());
}
...
void DatabaseWorkerPool<T>::CommitTransaction(SQLTransaction transaction)
{
    ...
    {
    case 0:
        WAR_LOG_DEBUG("sql.driver", "Transaction contains 0 queries. Not executing.");
        return;
    case 1:
        WAR_LOG_DEBUG("sql.driver", "Warning: Transaction only holds 1 query, consider removing Transaction context in code.");
        break;
    default:
        break;
    }
...
uint32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, uint8 numConnections)
{
    for (uint8 i = 0; i < numConnections; ++i)
    {
        ...
        {
            WAR_LOG_ERROR("sql.driver", "TrinityCore does not support MySQL versions below 5.1");
            return 1;
        }
        else
        {
...
template class WAR_DATABASE_API DatabaseWorkerPool<LoginDatabaseConnection>;
template class WAR_DATABASE_API DatabaseWorkerPool<WorldDatabaseConnection>;
template class WAR_DATABASE_API DatabaseWorkerPool<CharacterDatabaseConnection>;
template class WAR_DATABASE_API DatabaseWorkerPool<HotfixDatabaseConnection>;
