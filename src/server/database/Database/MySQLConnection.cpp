...
uint32 MySQLConnection::Open()
{
    ...
    if (!mysqlInit)
    {
        WAR_LOG_ERROR("sql.sql", "Could not initialize Mysql connection to database `%s`", m_connectionInfo.database.c_str());
    ...
    if (m_Mysql)
    {
        if (!m_reconnecting)
        {
            WAR_LOG_INFO("sql.sql", "MySQL client library: %s", mysql_get_client_info());
            WAR_LOG_INFO("sql.sql", "MySQL server ver: %s ", mysql_get_server_info(m_Mysql));
            // MySQL version above 5.1 IS required in both client and server and there is no known issue with different versions above 5.1
            // if (mysql_get_server_version(m_Mysql) != mysql_get_client_version())
            //     WAR_LOG_INFO("sql.sql", "[WARNING] MySQL client/server version mismatch; may conflict with behaviour of prepared statements.");
        }

        WAR_LOG_INFO("sql.sql", "Connected to MySQL database at %s", m_connectionInfo.host.c_str());
    ...
    {
        WAR_LOG_ERROR("sql.sql", "Could not connect to MySQL database at %s: %s", m_connectionInfo.host.c_str(), mysql_error(mysqlInit));
...
bool MySQLConnection::Execute(const char* sql)
{
...

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            ...
            WAR_LOG_INFO("sql.sql", "SQL: %s", sql);
            WAR_LOG_ERROR("sql.sql", "[%u] %s", lErrno, mysql_error(m_Mysql));
            ...
            WAR_LOG_DEBUG("sql.sql", "[%u ms] SQL: %s", getMSTimeDiff(_s, getMSTime()), sql);
    }
...
bool MySQLConnection::Execute(PreparedStatement* stmt)
{
    ...
    uint32 index = stmt->m_index;
    {
        ...
        WAR_LOG_DEBUG("sql.sql", "[%u ms] SQL(p): %s", getMSTimeDiff(_s, getMSTime()), m_mStmt->getQueryString(m_queries[index].first).c_str());
    ...
    for (auto itr = queries.begin(); itr != queries.end(); ++itr)
    {
        ...
        switch (itr->type)
        {
            case SQL_ELEMENT_PREPARED:
            {
                ...
                if (!Execute(stmt))
                {
                    WAR_LOG_WARN("sql.sql", "Transaction aborted. %u queries not executed.", (uint32)queries.size());
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ...
    if (!ret)
        WAR_LOG_ERROR("sql.sql", "Could not fetch prepared statement %u on database `%s`, connection type: %s.",
...
void MySQLConnection::PrepareStatement(uint32 index, const char* sql, ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        WAR_LOG_ERROR("sql.sql", "In mysql_stmt_init() id: %u, sql: \"%s\"", index, sql);
        WAR_LOG_ERROR("sql.sql", "%s", mysql_error(m_Mysql));
        m_prepareError = true;
    }
    else
    {
        ...
        {
            WAR_LOG_ERROR("sql.sql", "In mysql_stmt_prepare() id: %u, sql: \"%s\"", index, sql);
            WAR_LOG_ERROR("sql.sql", "%s", mysql_stmt_error(stmt));
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo, uint8 attempts /*= 5*/)
{
    switch (errNo)
    {
        ...
        case CR_SERVER_LOST_EXTENDED:
        {
            if (m_Mysql)
            {
                WAR_LOG_ERROR("sql.sql", "Lost the connection to the MySQL server!");
        ...
        {
            WAR_LOG_INFO("sql.sql", "Attempting to reconnect to the MySQL server...");
            ...
            if (!lErrno)
            {
                ...
                {
                    WAR_LOG_FATAL("sql.sql", "Could not re-prepare statements!");
                    std::this_thread::sleep_for(std::chrono::seconds(10));
                    std::abort();
                }

                WAR_LOG_INFO("sql.sql", "Successfully reconnected to %s @%s:%s (%s).",
            ...
            if ((--attempts) == 0)
            {
                ...
                WAR_LOG_FATAL("sql.sql", "Failed to reconnect to the MySQL server, "
        ...
        case ER_NO_SUCH_TABLE:
            WAR_LOG_ERROR("sql.sql", "Your database structure is not up to date. Please make sure you've executed all queries in the sql/updates folders.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            std::abort();
            return false;
        case ER_PARSE_ERROR:
            WAR_LOG_ERROR("sql.sql", "Error while parsing SQL. Core fix required.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            std::abort();
            return false;
        default:
            WAR_LOG_ERROR("sql.sql", "Unhandled MySQL errno %u. Unexpected behaviour possible.", errNo);
            return false;
    }
}
