...
public:
    ...
    AsyncAcceptor(boost::asio::io_service& ioService, std::string const& bindIp, uint16_t port) :
    ...
    void AsyncAcceptWithCallback()
    {
        ...
        _acceptor.async_accept(*socket, [this, socket, threadIndex](boost::system::error_code error)
        {
            if (!error)
            {
                ...
                {
                    WAR_LOG_INFO("network", "Failed to initialize client's socket %s", err.what());
                }
            }
...
void AsyncAcceptor::AsyncAccept()
{
    _acceptor.async_accept(_socket, [this](boost::system::error_code error)
    {
        if (!error)
        {
            ...
            {
                std::cout << "network: Failed to retrieve client's remote address " << err.what();
            }
        }
